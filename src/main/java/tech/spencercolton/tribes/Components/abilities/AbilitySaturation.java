...
public class AbilitySaturation extends Ability {
    ...
    public void run() {
        ...
        this.p.getActivePotionEffects().stream().filter(pe -> pe.getType().equals(PotionEffectType.SATURATION) && pe.getAmplifier() != this.multi).forEach(pe -> this.p.removePotionEffect(PotionEffectType.SATURATION));
